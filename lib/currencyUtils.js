import { safeFetch } from './onlineDetection';
...
export const getCurrencyFromLocation = async (latitude, longitude) => {
  try {
    ...
    if (error.name === 'OfflineError') {
      console.log('🌍 Skipping currency location detection - device is offline');
    } else {
...
export const getCurrencyFromCountryName = (countryName) => {
  ...
  const nameToCode = {
    // Americas
    'United States': 'US', 'Canada': 'CA', 'Mexico': 'MX', 'Guatemala': 'GT', 'Belize': 'BZ', 'El Salvador': 'SV', 'Honduras': 'HN',
    'Nicaragua': 'NI', 'Costa Rica': 'CR', 'Panama': 'PA', 'Cuba': 'CU', 'Dominican Republic': 'DO', 'Haiti': 'HT',
    'Jamaica': 'JM', 'Trinidad and Tobago': 'TT', 'Bahamas': 'BS', 'Barbados': 'BB', 'Saint Lucia': 'LC', 'Saint Kitts and Nevis': 'KN',
    'Saint Vincent and the Grenadines': 'VC', 'Grenada': 'GD', 'Dominica': 'DM', 'Antigua and Barbuda': 'AG', 'Saint Pierre and Miquelon': 'PM',
    'Greenland': 'GL', 'Bermuda': 'BM', 'Puerto Rico': 'PR', 'Argentina': 'AR', 'Chile': 'CL', 'Colombia': 'CO',
    'Peru': 'PE', 'Venezuela': 'VE', 'Uruguay': 'UY', 'Paraguay': 'PY', 'Bolivia': 'BO', 'Ecuador': 'EC', 'Guyana': 'GY', 'Suriname': 'SR',
    'French Guiana': 'GF',

    // Europe
    'United Kingdom': 'GB', 'Ireland': 'IE', 'Iceland': 'IS', 'Norway': 'NO', 'Sweden': 'SE', 'Finland': 'FI', 'Denmark': 'DK',
    'Faroe Islands': 'FO', 'Estonia': 'EE', 'Latvia': 'LV', 'Lithuania': 'LT', 'Poland': 'PL', 'Germany': 'DE', 'Netherlands': 'NL', 'Belgium': 'BE',
    'Luxembourg': 'LU', 'France': 'FR', 'Monaco': 'MC', 'Switzerland': 'CH', 'Liechtenstein': 'LI', 'Austria': 'AT', 'Czechia': 'CZ', 'Czech Republic': 'CZ',
    'Slovakia': 'SK', 'Hungary': 'HU', 'Romania': 'RO', 'Bulgaria': 'BG', 'Slovenia': 'SI', 'Croatia': 'HR', 'Italy': 'IT', 'Malta': 'MT',
    'Spain': 'ES', 'Portugal': 'PT', 'Andorra': 'AD', 'San Marino': 'SM', 'Vatican City': 'VA', 'Kosovo': 'XK', 'Albania': 'AL',
    'North Macedonia': 'MK', 'Macedonia': 'MK', 'Serbia': 'RS', 'Bosnia and Herzegovina': 'BA', 'Montenegro': 'ME', 'Moldova': 'MD',
    'Ukraine': 'UA', 'Belarus': 'BY', 'Russia': 'RU', 'Gibraltar': 'GI', 'Isle of Man': 'IM', 'Jersey': 'JE', 'Guernsey': 'GG',

    // Middle East & North Africa
    'Turkey': 'TR', 'Cyprus': 'CY', 'Greece': 'GR', 'Georgia': 'GE', 'Armenia': 'AM', 'Azerbaijan': 'AZ',
    'Saudi Arabia': 'SA', 'United Arab Emirates': 'AE', 'Qatar': 'QA', 'Kuwait': 'KW', 'Bahrain': 'BH', 'Oman': 'OM', 'Yemen': 'YE',
    'Jordan': 'JO', 'Lebanon': 'LB', 'Israel': 'IL', 'Palestine': 'PS', 'Syria': 'SY', 'Iraq': 'IQ', 'Iran': 'IR',
    'Morocco': 'MA', 'Algeria': 'DZ', 'Tunisia': 'TN', 'Libya': 'LY', 'Egypt': 'EG', 'Sudan': 'SD', 'South Sudan': 'SS',

    // Sub-Saharan Africa
    'Ethiopia': 'ET', 'Eritrea': 'ER', 'Djibouti': 'DJ', 'Somalia': 'SO', 'Kenya': 'KE', 'Tanzania': 'TZ', 'Uganda': 'UG', 'Rwanda': 'RW', 'Burundi': 'BI',
    'DR Congo': 'CD', 'Democratic Republic of the Congo': 'CD', 'Republic of the Congo': 'CG', 'Congo': 'CG', 'Gabon': 'GA', 'Equatorial Guinea': 'GQ',
    'Central African Republic': 'CF', 'Cameroon': 'CM', 'Nigeria': 'NG', 'Ghana': 'GH', 'Ivory Coast': 'CI', "Cote d'Ivoire": 'CI', 'Côte d’Ivoire': 'CI',
    'Burkina Faso': 'BF', 'Niger': 'NE', 'Mali': 'ML', 'Senegal': 'SN', 'Guinea': 'GN', 'Guinea-Bissau': 'GW', 'Sierra Leone': 'SL',
    'Liberia': 'LR', 'Togo': 'TG', 'Benin': 'BJ', 'Cape Verde': 'CV', 'Cabo Verde': 'CV', 'São Tomé and Príncipe': 'ST', 'Sao Tome and Principe': 'ST',
    'Mauritania': 'MR', 'Western Sahara': 'EH', 'Madagascar': 'MG', 'Mauritius': 'MU', 'Seychelles': 'SC', 'Comoros': 'KM',
    'Mozambique': 'MZ', 'Zimbabwe': 'ZW', 'Zambia': 'ZM', 'Malawi': 'MW', 'Angola': 'AO', 'Namibia': 'NA', 'Botswana': 'BW',
    'South Africa': 'ZA', 'Lesotho': 'LS', 'Eswatini': 'SZ', 'Swaziland': 'SZ',

    // South, Central, East, Southeast Asia
    'India': 'IN', 'Pakistan': 'PK', 'Bangladesh': 'BD', 'Sri Lanka': 'LK', 'Nepal': 'NP', 'Bhutan': 'BT', 'Maldives': 'MV',
    'China': 'CN', 'Mongolia': 'MN', 'North Korea': 'KP', 'South Korea': 'KR', 'Japan': 'JP', 'Taiwan': 'TW',
    'Hong Kong': 'HK', 'Macau': 'MO', 'Myanmar': 'MM', 'Burma': 'MM', 'Thailand': 'TH', 'Laos': 'LA', 'Cambodia': 'KH', 'Vietnam': 'VN',
    'Malaysia': 'MY', 'Singapore': 'SG', 'Indonesia': 'ID', 'Philippines': 'PH', 'Brunei': 'BN', 'East Timor': 'TL', 'Timor-Leste': 'TL',
    'Kazakhstan': 'KZ', 'Kyrgyzstan': 'KG', 'Tajikistan': 'TJ', 'Uzbekistan': 'UZ', 'Turkmenistan': 'TM',

    // Oceania
    'Australia': 'AU', 'New Zealand': 'NZ', 'Papua New Guinea': 'PG', 'Fiji': 'FJ', 'Solomon Islands': 'SB', 'Vanuatu': 'VU', 'Samoa': 'WS',
    'Tonga': 'TO', 'Tuvalu': 'TV', 'Kiribati': 'KI', 'Nauru': 'NR', 'Palau': 'PW', 'Micronesia': 'FM', 'Marshall Islands': 'MH',
    'New Caledonia': 'NC', 'French Polynesia': 'PF', 'Wallis and Futuna': 'WF',
  };
