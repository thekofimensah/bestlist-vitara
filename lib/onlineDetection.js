...
export const safeFetch = async (input, init = {}) => {
  ...
  try {
    ...
    // If fetch fails, double-check online status and log context
    if (error.name === 'TypeError' && error.message.includes('fetch')) {
      const context = {
        ...
        userAgent: navigator.userAgent.includes('CapacitorWebView') ? 'CapacitorWebView' : 'Browser'
      };
      console.warn('🌐 [SafeFetch] Fetch failed - Context:', context);
    }
    throw error;
  }
};
...
export const shouldRetrySubscription = (retryCount, maxRetries) => {
  // Don't retry if offline (silent - no logging to reduce noise)
  if (isOffline()) {
    return false;
  }
...
// Utility to determine if we should load from cache only (offline-first mode)
export const shouldLoadFromCacheOnly = () => {
  return isOffline();
};
