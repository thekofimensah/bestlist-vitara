...
import { isOffline } from './onlineDetection';
...
export const logErrorEvent = async (payload = {}) => {
  try {
    // Skip error logging when offline to prevent cascade of failed requests
    if (isOffline()) {
      console.log('🌐 [ErrorTracking] Skipping error logging - device is offline');
      return;
    }
  ...
  if (typeof window.fetch === 'function' && !window.__fetchPatchedForErrorTracking) {
    ...
    window.fetch = async (input, init = {}) => {
      ...
      try {
        ...
        if (!isErrorEvents && !deviceOffline) {
          logErrorEvent({
            component: 'fetch',
            api_endpoint: typeof input === 'string' ? input : input?.url,
            http_status: null,
...
export const trackedFetch = async (input, init = {}) => {
  try {
    ...
    if (!res.ok) {
      const url = typeof input === 'string' ? input : input?.url;
      const deviceOffline = isOffline();
      if (!(typeof url === 'string' && url.includes('/rest/v1/error_events')) && !deviceOffline) {
        logErrorEvent({
          component: 'fetch',
          api_endpoint: url,
          http_status: res.status,
