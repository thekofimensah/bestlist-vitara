-- Error events table for comprehensive client-side error tracking
...
-- RLS: users can insert their own events; read is allowed to everyone for dashboards (adjust if needed)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'error_events' AND policyname = 'error_events_select_all'
  ) THEN
    CREATE POLICY error_events_select_all ON public.error_events
      FOR SELECT USING (true);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'error_events' AND policyname = 'error_events_insert_own'
  ) THEN
