...
// Best-effort file extension detection from URL
...
export const cacheRemoteImage = async (remoteUrl, userId) => {
  ...
  try {
    ...
    // On native, avoid base64 over the Capacitor bridge: use native download if available
    if (isNative && typeof Filesystem.downloadFile === 'function') {
      const ext = getExtFromUrl(remoteUrl, 'jpg');
      ...
      await Filesystem.downloadFile({ url: remoteUrl, path: filename, directory: Directory.Data });
      const index = await ensureIndexLoaded();
    ...
    // If native but no streaming download available, skip caching to prevent OOM
    if (isNative) {
      return null;
    }

    // Web path: fetch and persist as base64
    const res = await fetch(remoteUrl, { mode: 'cors', cache: 'force-cache' });
    const blob = await res.blob();
    const base64 = await blobToBase64(blob);
    const ext = (blob.type && blob.type.split('/')[1]) || getExtFromUrl(remoteUrl, 'jpg');
    const filename = `${dirPath}/${hashString(remoteUrl)}.${ext}`;
    try {
      await Filesystem.mkdir({ path: dirPath, directory: Directory.Data, recursive: true });
    } catch (_) {}
