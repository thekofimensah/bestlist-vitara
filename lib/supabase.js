...
export const signOut = async () => {
  try {
    ...
    // Clear all caches and persistent data
    await clearAllUserData(userId);

    console.log('✅ [SignOut] Successfully signed out and cleared all data');
    return { error: null };
  } catch (err) {
    console.error('❌ [SignOut] Exception during sign out:', err);
    return { error: err };
  }
};
...
const clearAllUserData = async (userId) => {
  ...
  try {
    ...
    const { clearQueue } = await import('./offlineQueue.js');
    const { Preferences } = await import('@capacitor/preferences');

    console.log('🧹 [SignOut] Starting comprehensive cache clear...');

    // 1. Clear offline queue
    clearPromises.push(
      clearQueue().catch(err => console.warn('Failed to clear offline queue:', err))
    );

    if (userId) {
      // 2. Clear user-specific Preferences data
      const userSpecificKeys = [
        `local_avatar_url_${userId}`,
        `local_user_stats_${userId}`,
        `local_basic_profile_${userId}`,
        `profile_posts_${userId}`,
        `social_counts_${userId}`,
        `user_achievements_cache_v1_${userId}`,
        `lists_data_${userId}`, // User-specific lists cache
        `word_suggestions_cache_${userId}`
      ];

      userSpecificKeys.forEach(key => {
        clearPromises.push(
          Preferences.remove({ key }).catch(err => console.warn(`Failed to clear ${key}:`, err))
        );
      });
      ...
      try {
        if (typeof localStorage !== 'undefined') {
          localStorage.removeItem(`listOrder_${userId}`);
          localStorage.removeItem('recentLocations'); // Recent locations for item creation
          
          // Clear list attributes cache for all lists (stored as listAttributes_${listId})
          // We'll clear these by pattern since we don't have the specific listIds
          for (let i = 0; i < localStorage.length; i++) {
            const key = localStorage.key(i);
            if (key && key.startsWith('listAttributes_')) {
    ...
    // 4. Clear global/shared caches
    const globalKeys = [
      'local_image_cache_index_v1', // Image cache index
      'offline_queue_v1',           // Offline queue (already cleared above but double-check)
      'offline_queue_status_v1',    // Queue status
      'word_suggestions_cache',     // Global word suggestions cache
      'all_achievements_cache_v1',  // All achievements cache
      'app_version'                 // Version tracking from updateManager
    ];

    globalKeys.forEach(key => {
      clearPromises.push(
        Preferences.remove({ key }).catch(err => console.warn(`Failed to clear global ${key}:`, err))
      );
    });
    ...
    // 5. Clear in-memory caches by calling clear functions from hooks
    try {
      // Clear feed caches
      const { clearOfflinePostsForUser } = await import('../hooks/useOptimizedFeed.js');
      if (userId) {
        clearOfflinePostsForUser(userId);
      }

      // Note: Other in-memory caches (like Map objects in hooks) will be cleared
      // automatically when the components re-initialize after auth state change.
      // The hooks useAuth, useAchievements, etc. will reset their internal state
      // when the user becomes null.
    } catch (err) {
      console.warn('Failed to clear feed caches:', err);
    }

    console.log('✅ [SignOut] Cache clearing completed');
  } catch (err) {
    console.error('❌ [SignOut] Error during cache clearing:', err);
  }
};
...
export const deleteItemAndRelated = async (itemId) => {
  try {
    console.log('🗑️ [deleteItemAndRelated] Starting optimized deletion for item:', itemId);
    ...
    console.log('🗑️ [deleteItemAndRelated] Found', postIds.length, 'related posts');

    if (postIds.length > 0) {
      // 🚀 OPTIMIZATION: Delete likes and comments in parallel for speed
      ...
      console.log('🗑️ [deleteItemAndRelated] Deleted related likes and comments in parallel');
      ...
      console.log('🗑️ [deleteItemAndRelated] Deleted posts');
    }
    ...
    console.log('✅ [deleteItemAndRelated] Deletion completed in', Math.round(endTime - startTime), 'ms');
    
    return { error: itemDeleteError || null };
  } catch (error) {
    console.error('❌ [deleteItemAndRelated] Error during deletion:', error);
    return { error };
  }
};
...
export const getOptimizedFeedPosts = async (feedType = 'following', batchSize = 8, offset = 0) => {
  try {
    ...
    // Determine which posts are saved by current user (batch)
    let savedPostIdsSet = new Set();
    try {
      const { data: savedListId } = await supabase.rpc('get_or_create_saved_items_list', { p_user_id: user.id });
      if (savedListId && postIds.length > 0) {
        const { data: savedItems } = await supabase
          .from('items')
          .select('saved_from_post_id')
          .eq('list_id', savedListId)
          .eq('is_saved_item', true)
          .in('saved_from_post_id', postIds);
        (savedItems || []).forEach(row => { if (row.saved_from_post_id) savedPostIdsSet.add(row.saved_from_post_id); });
      }
    } catch (e) {
      // Non-fatal
    }
    ...
    const formattedPosts = postsData.map(post => {
      ...
      return {
        ...
        user_saved: savedPostIdsSet.has(post.id),
...
export const getOptimizedFeedPostsLoadMore = async (feedType = 'following', batchSize = 8, offset = 0) => {
  try {
    ...
    // Determine which of these additional posts are saved by current user (batch)
    let savedPostIdsSet = new Set();
    try {
      const { data: savedListId } = await supabase.rpc('get_or_create_saved_items_list', { p_user_id: user.id });
      if (savedListId && postIds.length > 0) {
...
export const getOptimizedUserPosts = async (userId, batchSize = 8, offset = 0, includePrivate = false) => {
  try {
    console.log(`🚀 [getOptimizedUserPosts] Loading user posts (userId: ${userId}, offset: ${offset}, includePrivate: ${includePrivate})`);
    ...
    // Only filter by public posts if not including private ones
    if (!includePrivate) {
  ...
  try {
    ...
    try {
      if (!error && typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('user:followed', { detail: { followerId: user.id, followingId: userId } }));
      }
    } catch (_) {}
...
export const unfollowUser = async (userId) => {
  try {
    ...
    try {
      if (!error && typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('user:unfollowed', { detail: { followerId: user.id, followingId: userId } }));
      }
    } catch (_) {}
